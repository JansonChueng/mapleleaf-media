      ...
      <footer className="relative bg-charcoal border-t border-white/[0.04]">
        <div className="max-w-[80rem] mx-auto px-6 lg:px-12 py-16 lg:py-20">
            ...
            <div className="lg:col-span-3">
              ...
              <ul className="space-y-3">
                {FOOTER_LINKS.map((link) => (
                  <li key={link.href}>
                    <a href={link.href} className="text-white/30 hover:text-maple-red text-sm transition-colors duration-300">
                      {link.label}
                    </a>
                  </li>
                ))}
            ...
            <div className="lg:col-span-4">
              ...
              <ul className="space-y-4">
                {COMPANY_INFO.map((info) => (
                  <li key={info.label}>
                    <span className="block font-mono text-[10px] text-white/20 tracking-[0.1em] uppercase mb-1">{info.label}</span>
                    {info.isEmail ? (
                      <a href={`mailto:${info.value}`} className="text-white/40 hover:text-maple-red text-sm transition-colors duration-300">{info.value}</a>
                    ) : (
          ...
          <div className="mt-16 pt-8 border-t border-white/[0.03] flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="font-mono text-[10px] text-white/12 tracking-[0.08em]">让中国品牌被世界看见 · Made with Maple Leaf</p>
            <p className="font-mono text-[10px] text-white/10 tracking-[0.08em]">SZ · CN</p>
          </div>
        </div>
      </footer>
    </>
